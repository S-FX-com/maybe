# Self Hosting ObieBooks with Docker

This guide will help you setup, update, and maintain your self-hosted ObieBooks application with Docker Compose. Docker Compose is the most popular and recommended way to self-host the ObieBooks app.
...
```bash
# Download the sample compose.yml file from the ObieBooks Github repository
...
If everything is working, you will see the ObieBooks login screen.
...
Most self-hosting users will want the ObieBooks app to run in the background on their computer so they can access it at all times. To do this, hit `Ctrl+C` to stop the running process, and then run the following command:
...
The mechanism that updates your self-hosted ObieBooks app is the GHCR (Github Container Registry) Docker image that you see in the `compose.yml` file:
...
If you are trying to get ObieBooks started for the **first time** and run into database connection issues, it is likely because Docker has already initialized the Postgres database with a _different_ default role (usually from a previous attempt to start the app).

If you run into this issue, you can optionally **reset the database**.

**PLEASE NOTE: this will delete any existing data that you have in your ObieBooks database, so proceed with caution.**  For first-time users of the app just trying to get started, you're generally safe to run the commands below.

By running the commands below, you will delete your existing ObieBooks database and "reset" it.
